...
class MysqlPipeline(object):
    ...
    def process_item(self, item, spider):
        ...
        if result_dd:
            ...
            cusor.execute('insert into zp_dd value (null,%s)', (item['gzdd'],))
            cusor.execute('select id from zp_dd where dd_name=%s', (item['gzdd'],))
            result_dd = cusor.fetchone()
            dd_id = result_dd[0]
        #2. 查询职位名称id
        ...
        if result_list:
            ...
            parm = (zwmc_id, gsmc_id, dd_id, item['min_zwyx'], item['max_zwyx'], item['fbrq'], item['href'], zwlb_id,gsxz_id,gzjy_id,xl_id,item['zprs'],gshy_id,gsgmm_id)
